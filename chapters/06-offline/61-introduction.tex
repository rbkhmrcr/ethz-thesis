...
represented as \emph{tokens}, with each token encoding an \emph{owner} and a
\emph{value}. CBDCs are either \emph{wholesale} or \emph{retail}: the former is
a payment instrument accessible only to financial institutions to settle their
transactions, whereas the latter is available to the general public to conduct
retail payments.

With over 100 central banks currently exploring CBDC~\cite{cbdctracker}, and a
few launching pilot programs and developing regulatory frameworks, CBDC systems
may become a reality in the medium-term future. This interest is due in large
...
\emph{offline payments}. Offline payments are a pre-requisite to digital
...
specifically, offline payments must be \textbf{(1) fast:} an offline payment
should take only a few seconds to complete; \textbf{(2) transferable:} CBDCs
received offline can be used in subsequent offline payments without
restriction; and \textbf{(3) privacy-preserving:} the central bank and the
...
ability can be pursued through \emph{preventive} or \emph{deterrent} measures,
...
alternative is to leverage a \emph{secure element} so that these restrictions
...
it can accommodate a solution that uses ECDSA signatures to \emph{authenticate
CBDCs} and \emph{authorise offline payments}, such a solution fails to meet the
...
We instead rely on \textbf{(1) anonymous credentials} to allow the payer and
the payee to transact anonymously, such that no one else can learn their
identities, \textbf{(2) blind signatures}, and \textbf{(3) zero-knowledge
...
\emph{efficiently} identify double spenders, we use \textbf{(4) verifiable
...
\emph{computationally heavy} but requires no secret keys, and one that is
\emph{lightweight}, comparable to an ECDSA signature, and makes use of a
...
beginning with Chaum's solutions~\cite{Chaum1982,Chaum1988}, which focus on the
privacy of users and on double spending detection. Privacy is achieved through
...
\paragraph{Divisible e-cash.} Divisible e-cash~\cite{Okamoto1991,Okamoto1995}
allows a user to withdraw a token and split it among different recipients
across multiple independent transactions. This comes at the cost of higher
complexity for withdrawals, payments, and deposits, which scale with the token
value. This is partially addressed in~\cite{Canard2015PKC,Canard2015ACNS},
which achieve constant computation and communication complexity for
withdrawal and payment, though deposits still carry large communication and
computational costs. While divisibility is a useful feature, we leave it as
future work and focus instead on transferability.

\paragraph{Transferable e-cash.} \cite{Okamoto1991} and~\cite{Chaum1992}
present e-cash schemes that are both transferable and divisible, at the cost
of high computational complexity; we instead focus on efficiency and
scalability. Fuchsbauer et al.\ maintain a constant token size regardless of
...
token also become identifiable~\cite{Fuchsbauer2009}. Baldimtsi et al.\
propose a scheme where token size grows linearly with the number of
transfers, but which reveals only the double spender's
identity~\cite{Baldimtsi2015}. Owing to the cost of this approach, we instead
rely on a trusted auditor, which can be distributed, to de-anonymise payments
as needed.

\cite{Canard2008,Bauer2021} introduce coin transparency, which ensures a payee
cannot tell whether they are receiving a token they previously owned.
\cite{Blazy2011} achieves this through extensive use of zero-knowledge proofs
together with a trusted party that tracks every token in the system and can
identify double spending attempts. While this captures the strongest level of
...
The bounded-anonymity model of~\cite{camenisch2006balancing} keeps
transactions anonymous as long as each user interacts with a given merchant no
more than a fixed number of times; beyond this, the user becomes identifiable.
...
Bitcoin~\cite{poon2016bitcoin}: to improve latency and scalability, users
...
batched transaction committed to the ledger. A network of such channels lets
...
aims~\cite{kappos2021empirical}. Teechain instead has users fund an account
...
platform~\cite{brown2016corda,soderberg2019krona}, focusing on legal analysis
...
to online double-spending checks. The People's Bank of China has designed a
...
Reserve Board's report on offline payments~\cite{aboulaiz2024offline}
...
online to authorise it~\cite{lobo2021pix}.

\paragraph{Decentralised privacy-preserving payments.}
Peredi~\cite{kiayias2022peredi}, Platypus~\cite{wust2022platypus}, and
UTT~\cite{tomescu2022utt} give privacy-preserving token schemes that
distribute trust via threshold cryptography while providing accountability
and compliance guarantees, but do not address the offline setting. OPERA
enables offline payments and prevents double spending using one-time readable
memory~\cite{park2017opera}, but this technology is not widely available in
practice. Rial and Piotrowska distribute the role of the bank via threshold
issuance and consider divisible offline payments~\cite{Rial2023}, whereas we
focus on efficient transferability. PayOff~\cite{Beer2024} offers a
