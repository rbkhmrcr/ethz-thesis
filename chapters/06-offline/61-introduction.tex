...
beginning with Chaum's solutions~\cite{C:Chaum82,C:ChaFiaNao88}, which focus on the
...
\paragraph{Divisible e-cash.} Divisible e-cash~\cite{C:OkaOht91,C:Okamoto95}
...
value. This is partially addressed in~\cite{PKC:CPST15,ACNS:CPST15},
...
\paragraph{Transferable e-cash.} \cite{C:OkaOht91} and~\cite{EC:ChaPed92}
...
\cite{ACNS:CanGou08,PKC:BauFucQia21} introduce coin transparency, which ensures a payee
cannot tell whether they are receiving a token they previously owned.
\cite{AFRICACRYPT:BCFGST11} achieves this through extensive use of zero-knowledge proofs
...
The bounded-anonymity model of~\cite{SCN:CamHohLys06} keeps
...
Peredi~\cite{CCS:KiaKohSar22}, Platypus~\cite{CCS:WKDC22}, and
UTT~\cite{EPRINT:TBAAGP22} give privacy-preserving token schemes that
...
issuance and consider divisible offline payments~\cite{PoPETS:RiaPio23}, whereas we
