...
of the same user cannot be tied to one another, and unforgeability, so only
credentials that the user actually possesses will verify~\cite{EC:CamLys01}.
...
becomes impractical or even impossible if there are many issuers worldwide, with
only a subset relevant to any one user.
...
assumes that a user's key is certified, so that credentials cannot be issued to
a key the user does not control. That assumption is usually left implicit, or
...
strong-RSA assumption~\cite{EC:CamLys01,SCN:CamLys02}. A later pairing based
...
Credentials built on the BBS+ signature scheme support selective disclosure in a
...
Unlinkable proofs of knowledge over a subset of signed messages, building on the
Pointcheval-Sanders signature scheme, achieve the same generic group-model
...
maintaining user privacy during presentations. Indexed aggregate signatures
support anonymous credentials this way, with unforgeability under the
$q$-Asymmetric Double Hidden Strong Diffie-Hellman assumption, and anonymity
under DDH in the random oracle model~\cite{canard2011anonymous}.
...
issuers to be stateful, tracking which index they use per tag. A variant removes
this at the cost that the issuer key that grows with a fixed attribute limit.

We give an aggregate anonymous credential scheme achieving the same asymptotic
costs as ART-Sign, but from more standard assumptions (m-BCDH, q-SDH rather than
the generic group model) and without requiring issuers to be stateful or to
bound the number of attributes in advance.  \Cref{tab:agg-creds-related}
compares our scheme with several single- and multi-issuer credential systems,
reporting asymptotic complexities.
...
\bottomrule
  ...
  \end{tabular}
  %
  \caption{Comparison with existing single and multi-issuer anonymous credential
schemes. $n$ is the total number of attributes a scheme supports, $k$ the number
of attributes disclosed in a presentation, $|$cred$|$ is the size of a full
aggregated credential.  \cite{canard2011anonymous} provides aggregate signatures
under a shared index, but does not consider independent issuers in the sense we
require.} \label{tab:agg-creds-related} \end{table}
...
\paragraph{Layout.} \Cref{sec:agg-creds-definitions} states the problem and
...
\Cref{sec:agg-creds-summary} concludes.
