...
attributes.  When $\F{\ATS}$ is called through $\sf{aggregate}$, $\present$, or
$\verif$, it receives vector $\vec{\atv{}} = (\atv{1}, \ldots, \atv{N})$  such
...
\begin{proof}
  ...
  inputs (including secrets) of the honest parties, and at their behest,
  interacts with corrupt parties (i.e., adversary \adv), in the real-world,
  ...
  \begin{enumerate}
    ...
    \item $\issuer{i}$ is corrupt.  If the execution of $\Pi$ terminates with
      ...
      \atv{i}, \signature_i) \rightarrow 1$, then $\simu$ records entry
  ...
  \soksimsign(\td, \inst, \msg, \crs)$, which yields $\sokverify(\inst,
  \proof^{\star}, \crs)$ to return $\true$.
  ...
  user $\user$ sends, in the real world, to the honest registration authority
  $\RA$, $\simu$ (acting as $\RA$) first checks if $\user$ knows the secret key
  $\usk$ underlying $\pk$.  If not, then $\simu$ stops.  Otherwise, it executes
  ...
  real world, to an honest verifier $V$, $\simu$ (simulating $V$) calls $\bool
  \leftarrow \sokverify(\crs, \vec{\atv{}}, \msg, \proof)$.  We recall that
  $\atv{i} = \varnothing$ if the attribute value is not disclosed.

  If $\bool  = \false$,  $\simu$ sends $(\msg, \vec{\atv{}} = (\atv{1}, ...,
  \atv{N}), \proof)$ to $\F{\ATS}'$, which returns $(\verifend, \bool^\star)$.
  Two scenarios present themselves.  {\bf (1)} There exists a dishonest user
  ...
  dishonest.  {\bf (2)} $\forall i: \atv{i} \neq \varnothing$, issuers
  $\issuer{i}$ and users holding attribute values $\atv{i}$ are honest.

  In case of scenario {\bf (1)}, the output of $\F{\ATS}'$ is defined by
  $\simu$, which instructs $\F{\ATS}'$ to return $(\verifend, \false)$.  In
  case of scenario {\bf (2)}, $\F{\ATS}'$ outputs $(\verifend, \true)$.
  ...
  honestly,  $\sokverify$ will never output $\false$.  Thus, the output of
  $\F{\ATS}'$ matches that of $\Pi$.

  If $\bool = \true$, then thanks to the extractability property of the
  signature of knowledge $\sok$, $\simu$ extracts witness $\witness = (\tau,
  \usk, \pk, \psi, \signature)$  and then executes the following steps:
  ...
  \begin{itemize}
    ...
    \item Otherwise, $\simu$ sends to $\F{\ATS}'$ signature request $(\sign,
      ...
      As a result, $\simu$ receives $(\verifend, \bool)$ and outputs $\bool$ in
      turn.

  \end{itemize}

  In what follows, we show that if $\simu$ does not abort, then $\bool =
  \true$.
  ...
  one of two things: {\bf (1)} $\forall i : \atv{i} \neq \varnothing$, there
  exists an entry $\langle \star, \tau, \atv{i}, \star \rangle$; or {\bf (2)}
  $\exists j: \atv{j} \neq \varnothing$ such that there is no recorded entry
  $\langle \star, \tau, \atv{j}, \star \rangle$ but issuer $\issuer{j}$ is
  dishonest.  {\bf (1)} implies that $\simu$ issued credentials for $\user$ for
  ...
  \proof^{\star})$ will result in $\F{\ATS}'$ returning $(\verifend, \true)$.
  This matches the output of the verification in the real world.  {\bf (2)}
  ...
  $\F{\ATS}'$ is dictated by $\simu$.  $\simu$ sets this output to $\true$ to
  ...
  \usk, \pk, \psi, \signature)$.  This means that $\user$ has knowledge of a
  ...
  valid witness $\witness = (\tau, \usk, \pk, \psi, \signature)$.  This means
