...
by~\cite{C:AGHO11}, is optimal in terms of size, and existentially unforgeable
in the generic group model (GGM). The second, by~\cite{C:KilPanWee15}, produces
...
Abe, Groth, Haralambiev, and Ohkubo~\cite{C:AGHO11} introduced the first
...
threshold variant of~\cite{C:AGHO11} is \emph{$t$-additive secret sharing},
...
Kiltz, Pan, and Wee~\cite{C:KilPanWee15} introduced a structure-preserving
...
\paragraph{Thresholdising the signature.} As with~\cite{C:AGHO11}, we start
from a $t$-additive secret sharing of $\sk = (\mathbf{k}, \vec{{B}},
\vec{{X}}, \vec{{Y}})$ among the signers. To sign a vector of messages
