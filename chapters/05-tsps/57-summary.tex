...
structure-preserving signatures of Abe, Groth, Haralambiev, and
Ohkubo~\cite{C:AGHO11} and of Kiltz, Pan, and Wee~\cite{C:KilPanWee15}, together
with generic MPC functionalities over both field and group elements. Both
...
signatures. Our scheme built from~\cite{C:AGHO11} achieves optimally small
threshold signatures of three group elements, matching the size of its
non-threshold counterpart, while our scheme built from~\cite{C:KilPanWee15}
relies only on the standard SXDH assumption. In both cases, the offline phase
...
group elements~\cite{Ghosh2023}, and implemented previously proposed
threshold structure-preserving signature schemes to allow for a concrete
efficiency comparison. Extending our approach to other structure-preserving
