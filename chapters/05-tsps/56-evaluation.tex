...
MPC over $\Zp$ and over a generic group $\Group$. We benchmark two concrete
...
setting~\cite{Goyal2021}. Because it operates with an honest majority of
participants, ATLAS achieves unconditional security against both
honest-but-curious and malicious adversaries.

ATLAS builds on the protocol of~\cite{damgaard2007scalable}, improving its
round complexity by a factor of two. Since it operates in the honest-majority
...
party $\PKing$ is chosen, and all parties locally compute $[e]_{2t} = [a]_t
\cdot [b]_t + [r]_{2t}$ and send these values to $\PKing$, who reconstructs
$e$ and distributes it back to all parties. Every party then locally computes
$[c]_t = e - [r]_t$.

ATLAS relies on the observation that when $\PKing$ is honest, and the sharing
of $e$ that $\PKing$ distributes is degree $t$ rather than degree $2t$, the
corrupted parties receive $t$ shares of a random degree-$t$ sharing $[e]_t$
from $\PKing$, which are uniformly random and independent of the secret. So
for an honest $\PKing$, the protocol for computing and consuming $([r]_t,
[r]_{2t})$ can be optimised, and the responsibilities of $\PKing$ are
themselves distributed across all parties. ATLAS reduces the number of rounds
by half by combining ideas from Beaver triples with the techniques
of~\cite{damgaard2007scalable}, letting parties use one round of
communication to cover two rounds' worth of multiplication gates. For the
...
MASCOT~\cite{keller2016mascot} performs authenticated secure multiplication
of shared field elements in the maliciously secure dishonest-majority
setting. Secret-sharing based MPC relies on generating multiplication
...
outputs the product of the shared elements, or else aborts once malicious
...
\paragraph{Implementation.} Our implementation builds on
MP-SPDZ~\cite{keller2020mp}, a framework for implementing MPC with
pre-processing. MP-SPDZ is built for the paradigm of sharing elements of
...
of $\GroupOne$ and $\GroupTwo$. Extending MP-SPDZ with group operations,
concretely elliptic curve group operations, has been considered and
implemented in prior work~\cite{dalskov2020securing,Ghosh2023}.
...
all operations to take place over pairing-friendly curves. This was also
considered by Ghosh et al.~\cite{Ghosh2023}, who integrate the RELIC
toolkit~\cite{relictoolkit} into MP-SPDZ for operations over pairing-friendly
curves, and extend MP-SPDZ with shared computation over elements of the
resulting elliptic curve groups.

MP-SPDZ operates with a pre-processing (offline) phase and an online phase.
In our setting, $\offsign$ corresponds to the pre-processing phase, and
$\onsign$ and $\combine$ run in the online phase.
...
~\cite{keller2016mascot,Goyal2021}. ATLAS is designed to work in both the
semi-honest and malicious settings, but the implementation available in
MP-SPDZ captures only the semi-honest case~\cite{keller2020mp}. ATLAS and
...
similar operations. In both cases, $\F{\inv}$ is built from $\F{\mul}$, with
$\F{\randshare}$ taking the role of \texttt{input} and $\F{\combine}$ taking
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{ c || c | c | c || c | c | c }
   \toprule
    & Assumption & Message space & $|\sk|$ & $|\pk|$ & $|\sigma|$ & Verification cost \\
   \midrule
    ~\cite{crites2023threshold} & PS + ROM & $(\genOne^{\vec{m}}, \genTwo^{\vec{m}}) \in \GroupOne^l \times \GroupTwo^l$ &
    $ (l+1) | \Zp | $ & $ l | \GroupOne | + (l+1) | \GroupTwo | $ & $ 2 | \GroupOne | $ & $3l+2$ \\
    ~\cite{Mitrokotsa2024} & SXDH & $ \vec{M} \in \GroupOne^l$ &
    $ 2(l+1) | \Zp | $ & $ (l+1) | \GroupTwo | $ & $ 6 | \GroupOne | + | \GroupTwo | $ & $l+11$ \\
    Section~\ref{sec:tsps-abe} & q-type & $ \vec{M} \in \GroupOne^l$ &
    $ (l+1) | \Zp | $ & $ | \GroupOne | + l| \GroupTwo | $ & $ | \GroupOne | + 2 | \GroupTwo | $ & $l+4$ \\
    Section~\ref{sec:tsps-kiltz} & SXDH & $ \vec{M} \in \GroupOne^l$ &
    $ l |\Zp | + 6 | \GroupOne | $ & $ (l+6)| \GroupTwo | $ & $ 6 | \GroupOne | + | \GroupTwo |$ & $2l+10$ \\
   \bottomrule
  \end{tabular}
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{ c || c | c | c }
   \toprule
    & $\offsign$ & $\onsign$ & $\combine$ \\
   \midrule
    ~\cite{crites2023threshold} & 0 & 0 & $ 2 | \GroupOne |$ \\
    ~\cite{Mitrokotsa2024} & 0 & 0 & $ 6 | \GroupOne | + | \GroupTwo |$ \\
    Section~\ref{sec:tsps-abe} & $ n(l+4)|\Zp|$ & 0 & $| \GroupOne | + 2 | \GroupTwo |$ \\
    Section~\ref{sec:tsps-kiltz} & $ n(12|\Zp| + 6|\GroupOne|) $ & 0 & $6 | \GroupOne | + | \GroupTwo | $ \\
   \bottomrule
  \end{tabular}
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{ c || c || c | c || c | c }
   \toprule
    & Centralised & \multicolumn{2}{c||}{ATLAS~\cite{Goyal2021}} & \multicolumn{2}{c}{MASCOT~\cite{keller2016mascot}} \\
    & & $\offsign$ & $\onsign$ & $\offsign$ & $\onsign$ \\
   \midrule
...
Table~\ref{table:comp} compares our two schemes against~\cite{crites2023threshold}
and~\cite{Mitrokotsa2024} in terms of assumptions, message space, key and
signature size, and verification cost, while Table~\ref{table:comms} compares
...
signatures, parties may instead perform only the constant-time $\onsign$
locally, and submit their signature shares to a single party to perform
$\combine$, rather than all parties communicating shares and each
constructing a copy of the signature locally. The signature generation phase
shown in Figure~\ref{eval-online} combines $\onsign$ and $\combine$.
