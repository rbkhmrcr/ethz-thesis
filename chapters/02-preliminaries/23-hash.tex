...
adversary, has oracle access. The random oracle answers each fresh query 
with an independently and uniformly sampled output from the function's range, 
records the resulting input and output pair, and returns the same output on
repeated queries. In security proofs, a simulator may instead choose the output
assigned to particular queries. We refer to this as programming the random oracle.