...
\begin{description}
  ...
  \item[$\key \sample \PRFgen(\secparam)$ :] The key generation algorithm takes
the security parameter as input, and outputs a key $\key$.
...
computable for every $\key$ output by $\PRFgen(\secparam)$, and if it satisfies
pseudorandomness, which we define below. We use the standard syntax and security
notions for pseudorandom functions~\cite{KatLin14}.
...
function sampled uniformly at random from all functions $\domain \to \range$.
...
input $x \in \domain$ returns $\PRFeval(\key, x)$ if $b = 0$, and $R(x)$ if $b =
1$.
