...
way by cryptography. One that can is the ability to keep information private
from those with more power to observe or act on it.
...
Modern cryptography has enabled encrypted
communications that even truly powerful entities cannot read. This
protection rests on an asymmetry of effort, not a change in who holds power.
...
cryptography itself. But protecting the content of a message says nothing
about who is exchanging it, when, or how often.
...
used. Every answer reveals another question. Answering each new question has
driven successive waves of privacy research.
...
performing otherwise legitimate functions. This growing list of legitimate
but powerful observers raises an uncomfortable question: how should a
protocol treat them?
...
This thesis takes payments as its proving ground, but the underlying
question is broader: what must a system's participants be able to learn and
do, and what can be withheld from them regardless? Financial systems make
that question concrete, and unusually demanding. Beyond confidentiality,
they must resist double spending, since digital tokens, unlike physical
cash, are trivial to duplicate; remain efficient as the number of
participants grows, ideally without cost per participant growing linearly
with the size of the system; and continue to function for a user with no
network connection at all, which matters for anyone without continuous
access to power or connectivity. These are not properties a private
communication channel needs to have, and they are why a system that gets
privacy right for messaging does not automatically get it right for money.
...
examine this principle through several systems, each closing a specific gap
left by treating decentralisation as sufficient for privacy on its own.
