...
The protocol instantiation is described in full in~\Fref{fig:txid}. The
...
\begin{description}
  ...
  \item[$(\crs,\state) \sample \IDgen(\secparam)$ :] The registration authority
    ...
    in~\Fref{fig:bbs}, it can sign group elements directly, letting the
  ...
  \item[$\tx \sample \IDsubmit(\crs,\sk,\rc,\epoch,\payload)$ :] Each
    ...
    attaches two separate proofs: one, described in~\Fref{sec:tag-computation},
    that the tag was correctly derived from a properly committed tracing key;
    and one, described in~\Fref{sec:tx-auth}, that the committed key was signed
