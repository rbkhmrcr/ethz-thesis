...
benchmark the schemes of Section~\ref{sec:tsps-abe} and~\ref{sec:tsps-kiltz}.
...
group elements~\cite{CCS:Keller20}, which, to our knowledge, is the first
such extension. We benchmark two concrete frameworks built on this
extension: ATLAS, in the honest-majority setting, and MASCOT, in the
...
setting, achieving unconditional security against both honest-but-curious
and malicious adversaries~\cite{C:GLOPS21}. It builds on an existing
protocol, improving its round complexity by a factor of two by combining
ideas from Beaver triples with existing multiplication techniques, letting
parties use one round of communication to cover two rounds' worth of
multiplication gates~\cite{C:DamNie07}. In terms of the functionalities of
Section~\ref{sec:MPC}, addition is performed locally, since ATLAS is built
from a linear secret sharing scheme; multiplication is performed by
consuming correlated randomness prepared during pre-processing; and shares
are combined by broadcasting them to all parties.

\subsection*{MASCOT}\label{sec:MASCOT}

MASCOT performs authenticated secure multiplication of shared field elements
in the maliciously secure dishonest-majority setting, generating the
multiplication triples this relies on using oblivious transfer, extended to
the arithmetic setting, together with MACs and standard sacrifice
...
detected~\cite{CCS:KelOrsSch16}. In terms of the functionalities of
Section~\ref{sec:MPC}, addition and inversion are performed locally, since
MASCOT is also built over a linear secret sharing scheme; multiplication
consumes the triples generated via oblivious transfer; and shares are
combined by broadcasting them, with MACs checked to ensure the
...
MASCOT both implement the ideal functionality of Section~\ref{sec:MPC}:
