...
specifically, offline payments must satisfy three properties. First, they
must be fast: an offline payment should take only a few seconds to complete.
...
\paragraph{Divisible ecash.} Divisible ecash allows a user to withdraw a
...
Another system maintains a constant token size regardless of
...
token also become identifiable~\cite{CANS:FucPoiVer09}. 
There is also a scheme where token size grows linearly with the number of
...
rely on a trusted auditor, which can be distributed, to deanonymise payments
as needed.
...
\paragraph{Offchain settlement.} Payment channels such as the Lightning
network and Teechain provide offchain payments rather than offline
...
payee, then exchange small transactions offchain, with only the final
...
occasional synchronisation with the ledger, once, for example, the total
value transferred crosses a risk threshold. Staged offline systems exchange
value offline but do not settle it until the payee comes online.
...
area\footnote{Reported at \url{https://www.bog.gov.gh/news/design-paper-of-the-digital-cedi-ecedi/}.}
...
India\footnote{Reported at \url{https://rbi.org.in/scripts/FAQView.aspx?Id=169}.}, which
...
\paragraph{Decentralised payments.}
Peredi, Platypus, and
...
Section~\ref{sec:offline-construction} gives the construction, first at the level of
...
Section~\ref{sec:offline-security} proves the construction secure and discusses
its security in practical deployments. Section~\ref{sec:offline-evaluation} evaluates our
