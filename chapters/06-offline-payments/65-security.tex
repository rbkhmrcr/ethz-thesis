...
\subsection{Proof of \texorpdfstring{\Cref{thm:offline-construction-security}}{the
construction security theorem}}\label{sec:offline-proof-1}
...
during $\OPwithdraw$ and $\OPexecute$ as the output of $\ComCommit(\ck, \cdot,
\cdot)$.
...
ciphertext it produces is random. This is what makes its transcripts
independent of the identities and token identifiers it does not learn. For corrupt parties
...
\leftarrow \ComCommit(\ck, (\algo{uid}, v), s)$, and calls $(\OPwithdraw,
\algo{Int},
...
$\com_b \leftarrow \ComCommit(\ck, r_b, s_b)$ for fresh $r_b, s_b$, a random
...
of both parties. It computes $\rho' \leftarrow \ComCommit(\ck, \algo{uid}',
s')$ from
...
$(\algo{uid}, s)$ from $\Sigma$, computes $\rho \leftarrow \ComCommit(\ck,
...
$\ComOpen(\ck, \com_0, s') = \algo{uid}'$, giving $\rho \leftarrow
\ComCommit(\ck, \algo{uid}', s')$ with value $v$. Combined with $\rho
\leftarrow \ComCommit(\ck, \algo{uid}^*, s^*)$ with value $v^*$ from the prior
withdrawal, $v \neq v^*$
...
contradicting (P1). If $\algo{usr}^*$ is honest, $\com_0 \leftarrow
\ComCommit(\ck, \algo{uid}^*, s^*)$ was chosen by $\simu$, so $\algo{usr}$
cannot have guessed
...
$\algo{usr}'$'s behalf, provides $\rho' \leftarrow \ComCommit(\ck,
(\algo{uid}', v), s')$ from $\com_1$, and extends the maps.
...
Each $\rho_i$ is determined by $\com_i$ via $\rho_i \leftarrow \ComCommit(\ck,
...
\subsection{Proof of \texorpdfstring{\Cref{thm:offline-instantiation-security}}{the
instantiation security theorem}}\label{sec:offline-proof-2}
...
Assumption~\ref{asm:qmsdh}~\cite{RSA:PoiSan18}, and blind signing over
