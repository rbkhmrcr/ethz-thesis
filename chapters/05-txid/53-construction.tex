...
two is the point of the construction. Those schemes require the counter to
increase by exactly one per transaction and to leave no gaps, because the
auditor recovers transactions by walking the counter upwards from zero and
stopping at the first miss. A user must therefore serialise: the $(i+1)$-th
transaction cannot be built until the $i$-th has been fixed. Here the auditor
...
index within an epoch, and that constraint is enforceable locally: a user may
partition $[0,\Max)$ among concurrent workers in advance, or draw indices from a
local pool, and submit the resulting transactions in any order and in parallel.
A user who instead samples $\gamma$ uniformly should note that collisions appear
after roughly $\sqrt{\Max}$ transactions, and that a collision costs a rejected
transaction rather than a security failure, so explicit allocation is
preferable. Once a user has consumed all $\Max$ indices they must wait for the
next epoch, so $\Max$ is a per-user, per-epoch rate limit, and the choice of
$\Max$ trades that limit against the cost of tracing and the size of a first
grant.
...
tracing key as a whole, which the instantiation takes to be a tuple. The user
...
be compatible, in the sense that $\DS$ supports signatures on committed values:
the registration authority must be able to certify $\G(\sk)$ and $\G'(\utk)$
without learning $\sk$ or $\utk$, and a user must later be able to prove in zero
knowledge that a commitment $\com$ opens to the same $\utk$ that a credential
certifies. Instantiating $\DS$ and $\Com$ independently would leave $\rel_6$
below requiring a generic proof over the signature verification equation, which
is exactly the expense the instantiation avoids. Second, $\FE$ splits, as
described next.

\paragraph{The shape of $\FE$.} We do not require $\FE$ to be a pseudorandom
function in the ordinary sense, because the instantiation's epoch key is not
one. Instead we require that its output decomposes as
%
\begin{equation}\label{eq:fe-split}
%
  \ek = \FE(\utk,\epoch) = (\ek_\epoch,\ \mathsf{aux}),
%
\end{equation}
%
where $\ek_\epoch$ depends on the epoch and $\mathsf{aux}$ does not, subject to
two conditions:
%
\begin{enumerate}

  \item[(F1)] the map $\epoch \mapsto \ek_\epoch$, keyed by $\utk$, is a
pseudorandom function; and

  \item[(F2)] $\FT(\cdot,\gamma)$ is a pseudorandom function on domain
$[0,\Max)$ even when $\mathsf{aux}$ is given to the adversary, provided
$\ek_\epoch$ is not.

\end{enumerate}
%
Since $\mathsf{aux}$ is constant across epochs, it is sent once, at a user's
first grant, and treated as public thereafter; only $\ek_\epoch$ need be
transmitted for subsequent grants. Condition (F2) is what makes releasing it
harmless, and (F1) is what confines an auditor to the epochs they have been
granted. A construction in which $\FE$ is a PRF outright is the special case
$\mathsf{aux} = \varnothing$.
...
\end{gathered} \end{equation}
%
Note that $\rel_5$ covers both components of~\Cref{eq:fe-split}. In the
instantiation the $\mathsf{aux}$ clause is proved once, at a user's first grant,
and the $\ek_\epoch$ clause once per epoch.
%
...
\begin{equation}
%
  \inst_7 = (\FE, \FT, \ck, \com, \epoch, \Max, \Tag_\gamma), \qquad \witness_7
...
%
  \rel_7 = \{\ \ComOpen(\ck, \com, \utk, \openinfo) = 1 \ \wedge \ \ek =
\FE(\utk, \epoch) \\ \wedge \ \Tag_\gamma = \FT(\ek, \gamma) \ \wedge \ 0 \leq
\gamma < \Max\ \}.
%
\end{gathered} \end{equation}
%
...
at submission time. This is deliberate and costs nothing: a grant is addressed
to a named auditor for a named user, so there is no identity left to hide from
them, and an auditor who replays the credential elsewhere cannot produce the
accompanying $\rel_4$ proof, which is bound to a fresh nonce and requires
knowledge of $\sk$.
...
transactions the user submitted during that epoch by computing $\Tag_\gamma =
\FT(\ek, \gamma)$ for each index $\gamma \in [0,\Max)$ and retrieving entry
$\ledger[\Tag_\gamma]$ where present.
...
unforgeability, $\FE$ satisfies (F1) and $\FT$ satisfies (F2), $\G$ and $\G'$
are one way, and the zero-knowledge proof systems for $\rel_4$ through $\rel_7$
