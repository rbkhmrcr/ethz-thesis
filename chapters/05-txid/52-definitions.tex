...
algorithm, run interactively between a user and the registration authority on
input public parameters $\crs$ and state $\state$, outputs user secret key $\sk$
and registration credential $\regcred$ to the user.
...
and epoch $\epoch$, outputs epoch tracing key $\ek$ to the auditor.
  ...
  \item[$\tx \sample \IDsubmit(\crs,\sk,\regcred,\epoch,\payload)$ :] The
submission algorithm, on input public parameters $\crs$, user secret key $\sk$,
registration credential $\regcred$, epoch $\epoch$, and transaction payload
$\payload$, outputs transaction $\tx$, which the user sends to the ledger
operator.
...
algorithm, on input public parameters $\crs$, epoch tracing key $\ek$, and epoch
$\epoch$, returns the set of payloads of the transactions submitted under $\ek$
during $\epoch$.
