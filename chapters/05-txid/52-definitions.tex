...
registration credential $\regcred$, and epoch $\epoch$, outputs the user's
certified public tracing key
...
transaction payload $\payload$, outputs transaction $\tx$, which the user sends
to the ledger
operator.
...
honest parties and $\adv$ in a real execution of $\Pi$, on inputs supplied by an
environment $\mathcal{Z}$. Let
$\textsc{Exec}_{\idfu{\algo{trace}},\simu}(\secparam,\mathcal{Z})$ denote the
same for the ideal world, where the honest parties and $\simu$ interact with
$\idfu{\algo{trace}}$ under that same environment. We say that
$\Pi$ securely realises $\idfu{\algo{trace}}$ if the following holds: for every
PPT adversary $\adv$ there is a PPT simulator $\simu$ whose ideal-world output
no PPT environment $\mathcal{Z}$ can tell from the real one.

The distinction from universal composability matters, because our simulator
extracts witnesses by rewinding the adversary,
via~\Cref{prot:txid-tag-sigma,prot:txid-tx-nizk} and the forking lemma, which is
sound in the standalone setting but not in the UC one. Straight-line extraction,
and with it UC security, can be recovered two ways: apply the Fischlin transform
to both sigma protocols, or have the prover verifiably encrypt its witness under
a key in the CRS. Both cost proof size, and we do not pay it here.
