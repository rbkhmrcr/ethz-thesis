...
is~\Cref{lem:txid-fe-prf}. The group elements $\{W_\gamma\}$ that a tag
evaluation also needs are static across epochs and sit in $\PTK$ rather than in
$\ek$, which is the arrangement~\Cref{sec:txid-construction} asks for: had they
been emitted by $\FE$ they would repeat in every epoch, and $\FE$ could not have
been a pseudorandom function at all. Placing them in $\PTK$ costs nothing, since
$\PTK$ is certified at registration and already travels with the grant message.
...
$\FT(\PTK, \ek, \gamma) = \PRFeval((\ggen_D,\ek), \utk'', \gamma) = \pairing(\ek,
W_\gamma)$ for the $W_\gamma$ carried by $\PTK$. Which argument is secret differs
between the two views, and deliberately so: the Dodis-Yampolskiy analysis keys on
$\utk''$ and treats $\ek$ as a parameter, while~\Cref{lem:txid-ft-prf} keys on
$\ek$ and treats $\{W_\gamma\}$, and hence $\utk''$ implicitly, as public. Note which argument is the key: $\FT$ is keyed
...
permissions are indexed by $(\auditor,\user,\epoch)$ and an auditor is told which
...
first time it is authorised for this user; subsequent grants carry $\ek$ alone.
...
\FT(\PTK,\ek,\gamma) = \pairing(\ek,W_\gamma)$. Indices need not be used in order, so this step
...
reused for every later epoch. Every subsequent grant to the same auditor is
constant size: since $\widehat{W}$ and $\widehat{V}$ depend only on
$\{W_\gamma\}$, the third verification equation of $\rel_5$ need only be checked
at that auditor's first grant, and later grants drop $K$ and the vector,
leaving the two $\group{1}$ equations against the same certified $T$. Formally
...
\end{equation*}
%
The randomised epoch key $\ek^{\,s}$ is itself a witness and is never published;
only its image $\Pi \in \group{T}$ under the pairing with $\ggen_D$ is. This is
...
(for Pedersen binding and for soundness of the Bulletproofs range argument).  \end{theorem}
