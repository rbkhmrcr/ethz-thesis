...
We give two concrete instantiations of the black-box construction. The first,
which occupies most of this section, works in pairing based groups using
algebraic PRFs and tailored sigma protocols, and yields a transparent setup. The
second, in~\Cref{sec:txid-hash-instantiation}, takes both pseudorandom functions
to be ordinary hash functions and discharges the two PRF relations with a
general purpose zk-SNARK. The two make opposite trades, and we return to the
comparison once the first is in place.

In the algebraic instantiation, three of the four general relations involve
pairing equations
...
The public tracing key carries, alongside the image of $\utk$, the vector of
group elements that a tag evaluation inverts against:
%
\begin{equation}\label{eq:txid-ptk}
%
  \PTK = \G'(\utk) = \big(T,\ \{W_\gamma\}_{\gamma\in[0,\Max)}\big), \qquad T =
\gen_2^{\utk'}\cdot\gen_3^{\utk''}, \qquad W_\gamma =
\ggen_D^{1/(\utk''+\gamma)}.
%
\end{equation}
%
The first component is a perfectly hiding Pedersen commitment to the pair, and
the second is what makes $\{W_\gamma\}$ available to an auditor without
disclosing $\utk''$. Both are certified in the same credential, and neither
...
$\utk'$. The epoch tracing key is then the single group element given below:
%
\begin{equation}\label{eq:txid-ek}
%
  \ek = \FE(\utk, \epoch) = \HE(\epoch)^{\utk'} \in \group{1}.
%
\end{equation}
%
Every component of the output varies with the epoch, so $\FE$ is a pseudorandom
function in the ordinary sense of~\Cref{sec:prelims-prfs}, which
is~\Cref{lem:txid-fe-prf}. A tag evaluation also needs the group elements
$\{W_\gamma\}$, and those are static across epochs. They sit in $\PTK$ rather
than in $\ek$, which is the arrangement~\Cref{sec:txid-construction} asks for.
Had $\FE$ emitted them they would repeat in every epoch, and $\FE$ could not
have been a pseudorandom function at all. Putting them in $\PTK$ costs nothing,
since $\PTK$ is certified at registration and already travels with the grant
message.
We return below to why the auditor receives them rather than $\utk''$ itself.

\begin{lemma}\label{lem:txid-fe-prf}
  %
  Under the DDH assumption in $\group{1}$ (\Cref{asm:ddh-txid}) and modelling
$\HE$ as a random oracle, $\FE(\utk,\epoch) = \HE(\epoch)^{\utk'}$ is a secure
pseudorandom function, and remains so against a distinguisher given the whole of
$\PTK$.
%
\end{lemma}
...
\begin{assumptionbox}{Decisional Bilinear Diffie-Hellman Inversion with
published elements ($q$-mDBDHI)}{qmdbdhi-txid}
...
\group{2}$. The $q$-mDBDHI assumption holds if, given $(\gen, \ggen_D,
\{\ggen_D^{1/(x+\gamma)}\}_{\gamma\in[0,q)})$ for uniformly random $x \sample
\Zp$ and $A \sample \group{1}$, no PPT adversary can distinguish
$\{\pairing(A, \ggen_D^{1/(x+\gamma)})\}_{\gamma\in[0,q)}$ from
$\{\Delta_{\gamma}\}_{\gamma \in [0,q)}$ for uniformly random $\Delta_{\gamma}
\sample \group{T}$ with non-negligible advantage. Neither $A$ nor $x$ is given
to the adversary. We use it with $q = \Max$.

\end{assumptionbox}

A single instance suffices here, even though a user is traced over many epochs.
The base elements $\{\ggen_D^{1/(x+\gamma)}\}$ are shared across epochs, since
$\{W_\gamma\}$ is fixed at registration, but the epoch keys are not: each $\ek$
is uniform and independent of every other by~\Cref{lem:txid-fe-prf}, whose
statement hands the distinguisher the whole of $\PTK$. The two lemmas therefore
compose directly, and no multi-instance variant or hybrid over epochs is needed.

This is the decisional variant of the $q$-BDHI assumption~\cite{EC:BonBoy04b},
...
partially oblivious PRF, also built on the Dodis-Yampolskiy PRF with a secret
group element, is analysed under a strong Diffie-Hellman inversion assumption of
its own~\cite{EC:TCRSTW22}. Pairing-based constructions of this type are in any
case known not to admit both short proofs and a reduction to a simple
assumption~\cite{TCC:BHKU22}.

\begin{primitivebox}{The Dodis-Yampolskiy pseudorandom function}{dy-prf}

  \begin{description}

    \item[$(\pp,\key) \sample \PRFgen(\secparam)$ :] On input the security
parameter, sample a generator $\ggen_D \sample \group{2}$ and a group element $X
\sample \group{1}$, and return $\pp = (\ggen_D, X)$ and $\key = k \sample \Zp$.

    \item[$y \leftarrow \PRFeval(\pp, \key, x)$ :] On input public parameters
$\pp = (\ggen_D, X)$, key $\key = k$, and input $x \in \Zp \setminus \{-k\}$,
...
fixed public generator, and the Dodis-Yampolskiy key $k$ to $\utk''$, giving
$\FT(\PTK, \ek, \gamma) = \PRFeval((\ggen_D,\ek), \utk'', \gamma) =
\pairing(\ek,
W_\gamma)$ for the $W_\gamma$ carried by $\PTK$. Which argument is secret
differs
between the two views, and deliberately so: the Dodis-Yampolskiy analysis keys
on
$\utk''$ and treats $\ek$ as a parameter, while~\Cref{lem:txid-ft-prf} keys on
$\ek$ and treats $\{W_\gamma\}$, and hence $\utk''$ implicitly, as public. Note
which argument is the key: $\FT$ is keyed
by $\ek$, which is exactly the range of $\FE$, while $\{W_\gamma\}$ enters as a
public parameter.
...
rather than its key. The auditor receives $\{W_\gamma\}$ as part of $\PTK$, and
tracing is unaffected, since an auditor holding $\PTK$ and $\ek$ computes every
tag of the epoch directly as $\pairing(\ek, W_\gamma)$.
...
Giving the auditor the group elements $\{W_\gamma\}$ instead removes the need
for the scalar, and $\utk''$ never leaves the user. The epoch key $\ek$ blinds
the released outputs, and the tags rest on that blinding.

The elements are public parameters, not secrets held in trust. They are a
component of the certified $\PTK$, they are handed to every auditor a user
authorises, and the analysis below treats them as known to the adversary
throughout. Two colluding auditors who compare their copies learn only that they
were granted by the same user, which each of them already knew, since
permissions are indexed by $(\auditor,\user,\epoch)$ and an auditor is told
which
user it is authorised to trace. What they cannot do is combine their copies with
a tag from an epoch for which neither holds $\ek$, and that is exactly the
statement of~\Cref{lem:txid-ft-prf}.

\begin{lemma}\label{lem:txid-ft-prf} Let $\ek$ be uniform in $\group{1}$ and
unknown to the adversary, and let the public parameters
$\{W_\gamma\}_{\gamma\in[0,\Max)}$ be given to the adversary. Under the
$q$-mDBDHI assumption (\Cref{asm:qmdbdhi-txid}) with $q = \Max$, the function
$\FT(\PTK, \ek, \gamma) = \pairing(\ek, W_\gamma)$ is a secure pseudorandom
function on domain $[0,\Max)$.  \end{lemma}
...
version does not. An adversary able to obtain a signature on an arbitrary group
element could ask
...
in the bases $\gen_i$. The registration protocol below supplies exactly that, in
the shape of $\algoproof_4$. So $\rel_4$ is not optional hardening. It is a
precondition of the signature scheme's security in this
setting~\cite{SCN:AuSusMu06,EC:TesZhu23b}.
...
$\gen_4$ plays the role of the Pedersen hiding generator.
	...
	\item Set $\Max \leftarrow 2^{\ell}$, $\G'(\utk) := (T,
\{W_\gamma\}_{\gamma\in[0,\Max)})$ for $T = \gen_2^{\utk'}\cdot\gen_3^{\utk''}$
and $W_\gamma = \ggen_D^{1/(\utk''+\gamma)}$, $\FE(\utk,\epoch) :=
\HE(\epoch)^{\utk'}$, and $\FT(\PTK,\ek,\gamma) := \pairing(\ek, W_\gamma)$,
following the Dodis-Yampolskiy construction~\cite{PKC:DodYam05}.
...
indexed by tag.
    ...
    \item[$(\sk,\utk,\regcred) \sample \IDregister(\crs,\state)$ :]
Interactively
...
\leftarrow \gen_1^{\sk}$, $T \leftarrow \gen_2^{\utk'}\cdot\gen_3^{\utk''}$, and
$\PTK \leftarrow (T,\{W_\gamma\})$ for $W_\gamma =
\ggen_D^{1/(\utk''+\gamma)}$,
...
registration authority bundled as the single element $T$.
	...
	\item Return $\sk$ as the user's signing key, $\utk = (\utk',\utk'')$ as
their tracing key, and $\regcred \leftarrow (S,T,\signature)$ as their
registration credential.

      \end{enumerate}

    \item[$(\PTK,\ek) \sample \IDgrant(\crs,\sk,\utk,\regcred,\epoch)$ :]
Interactively between the user and an auditor, on a fresh nonce $\rand$ from the
auditor, do:

      \begin{enumerate}

	\item Parse $\regcred = (S,T,\signature)$ and $\utk = (\utk',\utk'')$.

	\item The user computes $\ek \leftarrow \HE(\epoch)^{\utk'}$, and
recalls $\{W_\gamma\}$ from $\PTK$. The auditor is sent $\{W_\gamma\}$ only the
first time it is authorised for this user. Every grant after that carries $\ek$
alone.
...
$\gen_2^{\eta}\cdot\gen_3^{\zeta} \checkcheck A\cdot T^{c}$ and
$\HE(\epoch)^{\eta} \checkcheck B\cdot\ek^{c}$. On a first grant to this
auditor, it additionally checks $\widehat{W}^{\zeta} \checkcheck
K\cdot\widehat{V}^{c}$, which is what establishes $\{W_\gamma\}$ well formed
against the same certified $T$.

      \end{enumerate}

    \item[$\tx \sample \IDsubmit(\crs,\sk,\utk,\regcred,\epoch,\payload)$ :] On
input $(\crs,\sk,\utk,\regcred,\epoch,\payload)$ from the user, do:

      \begin{enumerate}

	\item Parse $\regcred = (S,T,\signature)$, $\utk = (\utk',\utk'')$, and
compute $\ek =
\FE(\utk,\epoch) = \HE(\epoch)^{\utk'}$.

	\item Take a fresh, unused index $0 \leq \gamma < \Max$ from the user's
local pool for this epoch, and compute $\Tag_\gamma \leftarrow
\FT(\PTK,\ek,\gamma) = \pairing(\ek,W_\gamma)$. Indices need not be used in
order, so this step
...
$\payload$ against the key $\Tag_\gamma$.
    ...
    \item[$\{\payload\} \leftarrow \IDidentify(\crs,\PTK,\ek,\epoch)$ :] On
input $(\crs,\PTK,\ek,\epoch)$, do:

      \begin{enumerate}

	\item For each $0 \leq \gamma < \Max$, compute $\Tag_\gamma \leftarrow
\pairing(\ek,W_\gamma)$, for the $\{W_\gamma\}$ carried by $\PTK$, and retrieve
...
could set $\gen_3 = \gen_2^{w}$ for known $w$ and then open any $\com$ to
...
The elements $\{W_\gamma\}$ do not depend on the epoch. As a component of $\PTK$
they are therefore sent once per auditor, at that auditor's first grant, and
reused for every later epoch. Every later grant to the same auditor is constant
size. Since $\widehat{W}$ and
$\widehat{V}$ depend only on $\{W_\gamma\}$, the third verification equation of
$\rel_5$ need only be checked at that auditor's first grant. Later grants drop
$K$ and the vector, leaving the two $\group{1}$ equations against the same
certified $T$. Formally
these are two relations, $\rel_5$ and its restriction $\rel_5'$ to the first two
conjuncts, with $\inst_5$ shortened accordingly. The binding of $T$ ties both
proofs to one pair $(\utk',\utk'')$. Nothing here is state an auditor must
retain: $\{W_\gamma\}$ is certified public key material, and an auditor that has
discarded it can be sent it again with the next grant.
...
$q_{\hash}/p$ over the random oracle, where $q_{\hash}$ bounds the prover's hash
queries. Proving $\rel_5$ also proves the second component of $\PTK$ well formed
against the same $\utk''$ that $T$ commits to, which is what an auditor needs in
order to rely on $\{W_\gamma\}$ when it evaluates tags.
...
$\Tag_\gamma = \FT(\PTK, \ek, \gamma) = \pairing(\ek, W_\gamma)$, where the
$W_\gamma$ are the group elements carried by $\PTK$. Under the $q$-mDBDHI
assumption (\Cref{asm:qmdbdhi-txid}), $\FT$ is a PRF with $\{W_\gamma\}$ given
to the distinguisher, which is~\Cref{lem:txid-ft-prf}.  Note that $\utk''$ is a
hidden witness of $\rel_7$ below, so the auditor never needs it in order to
check a transaction.

$\rel_7$ is defined for $\crs_7 \leftarrow \NIZKgen(\secparam, \rel_7)$, with
$\crs_7$
...
\Tag_\gamma), \qquad \witness_7 = (\utk', \utk'', \openinfo, \ek, \gamma), \\
%
...
element ($\ek$) paired against a term whose $\group{2}$ exponent is also secret,
which would be expensive in practice.
...
\begin{equation*}
%
  U = \HE(\epoch)^s, \quad \Pi = \pairing\big(\ek^{\,s},\, \ggen_D\big), \quad X
= \Tag_\gamma^{\,s}, \quad \com_\gamma = \gen_3^{\gamma}\cdot\gen_4^{r_\gamma}.
%
\end{equation*}
%
The randomised epoch key $\ek^{\,s}$ is itself a witness and is never
published. Only its image $\Pi \in \group{T}$ under the pairing with $\ggen_D$
is. This is
deliberate, and we return to it after the protocol. The commitment $\com_\gamma$
is what carries $\gamma$ between the sigma protocol and the range argument
below, so that the two speak about the same value. Since $\Pi =
\pairing(\ek,\ggen_D)^s$ and $\Tag_\gamma^{\utk''+\gamma} =
\pairing(\ek,\ggen_D)$ when the tag is correctly formed, we have $\Pi =
X^{\utk''+\gamma}$. The user then proves the equivalent relation $\rel_7'$,
proved under the same $\crs_7$, with instance, witness, and relation given as
follows:
%
\begin{equation} \begin{gathered}
%
  \inst_7' = (\inst_7, U, \Pi, X, \com_\gamma), \qquad \witness_7' =
(\witness_7, s, r_\gamma), \\
%
  \rel_7' = \{\ \com = \gen_2^{\utk'}\cdot\gen_3^{\utk''}\cdot\gen_4^{\openinfo}
\ \wedge \ U = \HE(\epoch)^s \ \wedge \ \Pi = \pairing(U,\ggen_D)^{\utk'} \\
\wedge \ X = \Tag_\gamma^s \ \wedge \ \Pi = X^{\utk''}\cdot X^{\gamma} \\ \wedge
...
or $\group{T}$ between public bases. The single pairing $\pairing(U,\ggen_D)$ is
...
On input $\inst_7' = (\inst_7, U, \Pi, X, \com_\gamma)$ and $\witness_7' =
...
\HE(\epoch)^{\mu_s}$, $M = \pairing(U,\ggen_D)^{\mu_1}$, $C =
\Tag_\gamma^{\mu_s}$, $F = X^{\mu_2 + \mu_\gamma}$, and $A_\gamma =
\gen_3^{\mu_\gamma}\cdot\gen_4^{\mu_r}$.

	  \item Compute challenge $c \leftarrow \hash(\inst_7', A, B, M, C, F,
A_\gamma)$.
...
r_\gamma$.
	  ...
	  \item Return $\algoproof_7' = (U, \Pi, X, \com_\gamma, A, B, M, C, F,
...
1_{\group{1}}$ and $\Pi \neq 1_{\group{T}}$, compute challenge $c \leftarrow
\hash(\inst_7', A, B, M, C, F, A_\gamma)$, and return the result of the
following checks:
      %
	\begin{gather*} \gen_2^{\theta_1}\cdot\gen_3^{\theta_2}\cdot
\gen_4^{\theta_h} \checkcheck A \cdot \com^c, \quad \HE(\epoch)^{\theta_s}
\checkcheck B \cdot U^c, \quad \pairing(U,\ggen_D)^{\theta_1} \checkcheck M
\cdot \Pi^c, \\ \Tag_\gamma^{\theta_s} \checkcheck C \cdot X^c, \quad
X^{\theta_2 + \theta_\gamma} \checkcheck F \cdot \Pi^c, \\
...
First, the fifth equation has $X$ as its only base, so it binds the sum
...
blinding scalar $s$ was used for $U$ and for $X$, and hence that $X$ is a
blinding of the tag named in the instance rather than an unrelated $\group{T}$
element.

Third, the randomised epoch key $\ek^{\,s}$ never appears in the proof. Only
$\Pi = \pairing(\ek^{\,s},\ggen_D)$ does, and the third verification equation
speaks about it through $\pairing(U,\ggen_D)$, a $\group{T}$ base the verifier
computes for itself. Publishing $\ek^{\,s}$ as a $\group{1}$ element would give
a verifier everything $\Pi$ gives and more, since $\Pi$ is recoverable from it
by one pairing. Withholding it is what lets the zero-knowledge argument
of~\Cref{sec:rel7-equiv} rest on DDH in $\group{T}$ alone.

Fourth, the two non-degeneracy checks matter. If $s = 0$ then $U$, $\Pi$, and
$X$ all collapse to identities and the pairing equality says nothing. The check
$U \neq 1_{\group{1}}$ rules this out, since $\HE(\epoch)$ generates
$\group{1}$ with overwhelming probability over the random oracle. And if $\utk''
= -\gamma$ then a prover holding $\utk' = 0$ satisfies the pairing equality with
any tag at all. The check $\Pi \neq 1_{\group{T}}$ rules this out in turn. Both
are used in the soundness argument of~\Cref{sec:txid-security}.
...
$\crs_6 \leftarrow \NIZKgen(\secparam, \rel_6)$, with $\crs_6$ describing a hash
function
...
\begin{protocolbox}{Transaction authorisation proof}{txid-tx-nizk}
...
\begin{theorem}\label{thm:inst} The algebraic instantiation presented above
securely realises $\idfu{\algo{trace}}$, in the
...
assumption in $\group{1}$ (for $\FE$), the $q$-mDBDHI assumption with $q = \Max$
(for $\FT$), the Decisional Diffie-Hellman assumption in $\group{T}$ (for zero
knowledge of the tag argument), the $q$-Strong Diffie-Hellman assumption (for
the BBS+ signature scheme), and the discrete logarithm assumption in $\group{1}$
(for Pedersen binding and for soundness of the Bulletproofs range argument).
\end{theorem}
...
\subsection{An alternative instantiation from hash based
PRFs}\label{sec:txid-hash-instantiation}

Nothing in~\Cref{sec:txid-construction} requires $\FE$ or $\FT$ to be algebraic.
The construction asks only that both be pseudorandom against a distinguisher
holding $\PTK$, and the work done above in rewriting $\rel_6$ and $\rel_7$ into
sigma-provable form exists solely because we wanted to avoid a general purpose
proof system. If we accept one, both functions can be ordinary hash functions
and the instantiation becomes far shorter to state. We describe it here, and
benchmark it alongside the algebraic instantiation
in~\Cref{sec:txid-evaluation}.

We use independent hash functions $\hashfs$ for Fiat-Shamir challenges, $\HE$ for
epoch key derivation, and $\HT$ for tags, and instantiate the components as
follows:
%
\begin{enumerate}

  \item The one-way functions are $\G(\sk) = \gen_1^{\sk}$ and $\G'(\utk) =
\gen_2^{\utk}$, for a tracing key $\utk \in \Zp$ that is now a single scalar
rather than a pair. The public tracing key $\PTK = \G'(\utk)$ is one group
element and carries nothing else.

  \item The pseudorandom functions are $\FE(\utk,\epoch) = \HE(\utk,\epoch)$ and
$\FT(\PTK,\ek,\gamma) = \HT(\ek,\gamma)$, with $\HE$ and $\HT$ modelled as
pseudorandom functions keyed by their first argument. Note that $\FT$ ignores
$\PTK$ entirely. This is the degenerate case of the requirement
of~\Cref{sec:txid-construction}, where the public parameter of $\FT$ is empty.
It is also the one thing the algebraic instantiation cannot do, since evaluating
a tag there needs the vector $\{W_\gamma\}$, and that vector has to live
somewhere that does not vary with the epoch.

  \item Registration credentials use the BBS+ signature scheme, now certifying
the two scalars $(\sk,\utk)$ rather than three.

  \item Commitments use the Pedersen commitment scheme.

  \item The proof systems for $\rel_5$ and $\rel_7$ are instantiated with
zk-SNARKs. We use Groth16~\cite{EC:Groth16}, whose proofs are three group
elements regardless of the circuit and whose verification is 3 pairing
evaluations and one multi-exponentiation over the public inputs.

  \item The signature of knowledge systems for $\rel_4$ and $\rel_6$ are
instantiated with sigma protocols compiled by Fiat-Shamir under $\hashfs$, exactly
as in the algebraic instantiation.

\end{enumerate}

The relations are proved as~\Cref{sec:txid-construction} states them. There is
no $\rel_7'$, no randomised $(U,\Pi,X)$, and no separate Bulletproofs argument,
because the range check $0 \leq \gamma < \Max$ is simply a constraint of the
$\rel_7$ circuit. The 24KB first grant disappears with $\{W_\gamma\}$: a grant
carries one hash output and a constant-size proof. Tracing is $\Max$ evaluations
of $\HT$ rather than $\Max$ pairings, which is cheaper by orders of magnitude.

What is given up is the setup. Groth16 needs a structured reference string per
relation, so $\crs_5$ and $\crs_7$ come from a trusted setup, and the property
that no party samples anything, which the algebraic instantiation goes to some
length to obtain, is lost. The prover is also substantially slower, as
\Cref{sec:txid-evaluation} shows. Which instantiation is preferable therefore
depends on whether a deployment can tolerate a trusted setup, and on whether it
is proving cost or verification cost that binds.

\begin{theorem}\label{thm:hash-inst} The hash based instantiation presented
above securely realises $\idfu{\algo{trace}}$, in the sense of the standalone
model of~\Cref{sec:txid-definitions}, in the random oracle model for $\hashfs$,
under the $q$-Strong Diffie-Hellman assumption (for the BBS+ signature scheme),
the discrete logarithm assumption in $\group{1}$ (for Pedersen binding), the
knowledge soundness and zero knowledge of Groth16, and the assumption that $\HE$
and $\HT$ are pseudorandom functions.  \end{theorem}

The proof is~\Cref{thm:txid-bb} applied directly. Conditions (P1), (P2) and (P4)
are discharged by BBS+, Pedersen, and the two proof systems, and (P3) by the
assumption on $\HE$ and $\HT$, which needs no argument of its own here because
$\PTK$ carries nothing that a distinguisher could use. In particular no
assumption specific to the tracing mechanism appears: the DDH and $q$-mDBDHI
assumptions of~\Cref{thm:inst} were artefacts of the algebraic PRFs, not of the
construction.
