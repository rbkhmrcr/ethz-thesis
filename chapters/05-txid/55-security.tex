...
a distinguisher holding the user's public tracing key $\PTK$. This is the
requirement of~\Cref{sec:txid-construction}: $\PTK$ is a public parameter of
$\FT$ rather than key material, so nothing it carries need be kept secret, and
because it is fixed for the lifetime of the user it is deliberately kept out of
the output of $\FE$. In the instantiation the two statements are
...
proofs. Pseudorandomness of $\FT$ is exactly what licenses this: the tags of an
epoch are pseudorandom to an adversary holding $\PTK$ but not $\ek$, which is
the position of every corrupt auditor here. So by (P2), (P3), and (P4), this is
indistinguishable from submitting transactions in an execution of
$\Pi$.
...
computes the tag as $\FT(\PTK_\user,\ek,\gamma)$ so that the corrupt auditor's tracing
...
epoch and reused thereafter, so no two epochs can collide on a programmed value
of $\HE$.
...
(\Cref{asm:qsdh-bbs}), which we already assume for the BBS+ signature scheme. Since
$\utk''$ is fixed once at registration and never leaves the user, and the group
elements $\{W_\gamma\}$ given to an auditor are certified public parameters
identical in every epoch, nothing in a grant links one epoch to another beyond
what $\ek$ itself discloses to auditors already holding permission for that
epoch.
...
published, the simulator would have to sample it in $\group{1}$ and the argument
would need DDH in $\group{1}$ in the presence of $\ek$, a strictly stronger
requirement, since $\Pi$ is recoverable from $\ek^{\,s}$ by one pairing but not
conversely. Note that $X$ needs no separate assumption, since it is a function
of public data and of the simulator's own $s$.
...
existentially unforgeable under the $q$-SDH assumption (\Cref{asm:qsdh-bbs}). The
