...
binding, which the extraction step of (P4) relies on.
  ...
  \item[(P3)] $\FE$ satisfies condition (F1) and $\FT$ satisfies condition (F2)
of~\Cref{sec:txid-construction}. Note that we do \emph{not} assume $\FE$ is a
pseudorandom function: its output contains a component $\mathsf{aux}$ that is
identical in every epoch, so it cannot be. What (F1) gives is that the
epoch-varying component $\ek_\epoch$ is pseudorandom, and what (F2) gives is
that the tags of an epoch remain pseudorandom even once $\mathsf{aux}$ is
public, so long as $\ek_\epoch$ is not. In the instantiation these are
...
the standalone one rather than universal composability; see the discussion there
for how straight-line extraction would be obtained instead.
...
epoch-varying component of the epoch key by a uniform value, and simulates the
proof for relation $\rel_5$.

  Concretely, writing $\ek = (\ek_\epoch, \mathsf{aux})$ as
in~\Cref{eq:fe-split}, $\simu$ samples $\ek_\epoch$ uniformly and sets
$\permsimu[(\auditor,\user,\epoch)] \leftarrow (\ek_\epoch,
\mathsf{aux}_\user)$, where $\mathsf{aux}_\user$ is sampled once per user and
reused in every epoch. Reusing it is not an optimisation but a requirement:
$\mathsf{aux}$ is by definition identical across epochs, so a simulator that
drew a fresh one per grant would be trivially distinguishable by a corrupt
auditor holding grants for two epochs.

  When such a user submits a transaction, $\simu$ computes the tag as
$\FT(\ek,\gamma)$ under this substituted key, so that the corrupt auditor's
tracing still succeeds, and simulates the zero-knowledge proof for relation
$\rel_7$. By condition (F1) on $\FE$ (P3), applied as a PRF across the epochs
for which the auditor holds no permission, and by (P4), this game is
...
proofs. Condition (F2) is exactly what licenses this: the tags of an epoch are
pseudorandom to an adversary holding $\mathsf{aux}$ but not $\ek_\epoch$, which
is the position of every corrupt auditor here. So by (P2), (F2) via (P3), and
(P4), this is indistinguishable from submitting transactions in an execution of
$\Pi$.
...
computes the tag as $\FT(\ek,\gamma)$ so that the corrupt auditor's tracing
succeeds, and simulates the rest of the transaction as in Game 4.
...
we obtain $\Tag_{\gamma} = \FT(\ek, \gamma)$ with $\ek = \FE(\utk, \epoch)$ and
...
set $\{\FT(\FE(\utk,\epoch),\gamma)\}_{\gamma \in [0,\Max)}$ for that user's own
...
\subsection{Proof of~\Cref{lem:txid-fe-prf}}
...
\gen^{a \cdot {r_i}} = (\gen^a)^{r_i}$ for fresh random $r_i \sample \Zp$.
    ...
    \item For each query $\epoch_i$ to the PRF oracle, return $Z^{r_i}$.

  \end{enumerate}

  If $Z = \gen^{ab}$, then $Z^{r_i} = \HE(\epoch_i)^{b}$, so $\simu$ has
constructed the real function with key encoded in $\gen^b$, and responses are
...
%
  \begin{equation*}
%
    \Adv[\adv_{\PRF}(\FE)] \leq \Adv[\adv_\mathsf{DDH}(\simu)] +
\frac{q_{\HE}^2}{2p},
%
  \end{equation*}
%
  where $q_{\HE}$ bounds the number of random oracle queries and the second term
accounts for a collision among the $r_i$, which would make two epochs share a
programmed value of $\HE$.

\end{proof}

\subsection{Proof of~\Cref{lem:txid-ft-prf}}

\begin{proof}

  Let $\adv$ distinguish the tag vectors of $n$ epochs from random with
advantage $\varepsilon$. We use $\adv$ to build a simulator $\simu$ that breaks
$(\Max,n)$-mDBDHI.

  $\simu$ receives a $(\Max,n)$-mDBDHI challenge $(\gen, \ggen_D,
\{\ggen_D^{1/(x+\gamma)}\}_{\gamma\in[0,\Max)},
\{\Delta_{i,\gamma}\}_{i\in[1,n],\gamma\in[0,\Max)})$, where either
$\Delta_{i,\gamma} = \pairing(A_i, \ggen_D^{1/(x+\gamma)})$ for every
$(i,\gamma)$ and hidden $A_i \sample \group{1}$, or every $\Delta_{i,\gamma}
\sample \group{T}$ independently. It forwards
$\{\ggen_D^{1/(x+\gamma)}\}_{\gamma\in[0,\Max)}$ to $\adv$ as the published
group elements $\{W_\gamma\}$, and $\{\Delta_{i,\gamma}\}_\gamma$ as the tag
vector of epoch $i$, implicitly setting $\ek'_i = A_i$ and $\utk''^\star = x$,
and echoes $\adv$'s guess.

  The multi-instance form is what the protocol needs, and it is not obtained for
free from the single-instance one by a hybrid: the $n$ instances share the same
$x$, because $\{W_\gamma\}$ is fixed once at registration and reused in every
epoch, so a hybrid that swapped one epoch at a time would have to produce the
other epochs' vectors under that same unknown $x$. The rerandomisation given
after~\Cref{qmdbdhi-txid} supplies exactly that, which is why the reduction is
tight in $n$.
...
$\simu$ hands to $\adv$ is exactly $\{\FT(\ek_i,\gamma)\}_{\gamma\in[0,\Max)}$
in each epoch $i$, under key $\utk''^\star$. If the challenge is random, every
entry is independent and uniform in $\group{T}$. We then have the following
advantage:
%
  \begin{equation*}
%
    \Adv[\adv_{\PRF}(\FT)] \leq
\Adv[\adv_{(\Max,n)\text{-}\mathsf{mDBDHI}}(\simu)].
%
  \end{equation*}
%
  That each $\ek'_i$ is uniform in $\group{1}$, and independent across the
epochs for which $\adv$ holds no permission, is~\Cref{lem:txid-fe-prf}.

  We also note that the group elements $\{W_\gamma\}$ do not compromise $\utk''$
itself: recovering $\utk''$ from them would break the $q$-SDH assumption
(\Cref{qsdh-bbs}), which we already assume for the BBS+ signature scheme. Since
$\utk''$ is fixed once at registration and never leaves the user, and the group
elements $\{W_\gamma\}$ published to an auditor are identical in every epoch, no
...
%
$\rel_7'$ is defined for instance $\inst_7' = (\inst_7, U, V, \Xi, \com_\gamma)$
...
= \HE(\epoch)^s \ \wedge \ V = U^{\utk'} \\ \wedge \ \Xi = \Tag_\gamma^s \
\wedge \ \pairing(V,\ggen_D) = \Xi^{\utk''}\cdot\Xi^{\gamma} \\ \wedge \
...
r_\gamma \sample \Zp$. Set $U = \HE(\epoch)^{s}$, $V = \ek'^{s}$, $\Xi =
\Tag_\gamma^{s}$, and $\com_\gamma = \gen_3^{\gamma}\cdot\gen_4^{r_\gamma}$.
The equalities for $\com$, $U$, $\Xi$, $\com_\gamma$, and the range of $\gamma$
hold by construction, and $V = (\HE(\epoch)^{\utk'})^{s} = U^{\utk'}$ gives the
one for $V$. For the pairing equality, $\Tag_\gamma = \pairing(\ek',
\ggen_D^{1/(\utk''+\gamma)})$ gives $\Tag_\gamma^{\utk''+\gamma} =
\pairing(\ek',\ggen_D)$, which gives the following:
%
  \begin{equation*}
%
    \Xi^{\utk''}\cdot\Xi^{\gamma} = \big(\Tag_\gamma^{\utk''+\gamma}\big)^{s} =
\pairing(\ek',\ggen_D)^{s} = \pairing(\ek'^{s},\ggen_D) = \pairing(V,\ggen_D).
%
  \end{equation*}
...
extracted $\gamma$ is the value the range argument speaks about.
  ...
  From $U = \HE(\epoch)^{s}$ and $V = U^{\utk'}$ we get $V =
\HE(\epoch)^{s\cdot\utk'} = \ek'^{s}$. With $\Xi = \Tag_\gamma^{s}$, the pairing
equality then reads as follows:
%
  \begin{equation*}
%
...
%
  \end{equation}
%
  The verifier also rejects unless $V \neq 1_{\group{1}}$. As $V = \ek'^{s}$
with $s \neq 0$, this forces $\ek' \neq 1_{\group{1}}$, and non-degeneracy of
the pairing gives $\pairing(\ek',\ggen_D) \neq 1_{\group{T}}$.
...
out $\utk'' = -\gamma$.
  ...
  Finally, the fifth verification equation is
$\Xi^{\theta_2}\cdot\Xi^{\theta_\gamma} = F\cdot\pairing(V,\ggen_D)^{c}$. As
$\Xi^{\theta_2}\cdot\Xi^{\theta_\gamma} = \Xi^{\theta_2+\theta_\gamma}$, this
...
$\inst_7$ and without $\witness_7$, produces a transcript $(U, V, \Xi,
\com_\gamma, \algoproof_7')$ computationally indistinguishable from an honestly
generated one.

  \begin{description}

    \item[Simulating $(U, V, \Xi, \com_\gamma)$.] $\simu$ samples $s, r_\gamma
\sample \Zp$ and sets $U = \HE(\epoch)^{s}$ and $\Xi = \Tag_\gamma^{s}$.  It can
...
range $\gamma^\star$. It then samples $V \sample \group{1}$ uniformly, as it
cannot compute $\ek'^{s}$.

      The only deviation from the real distribution is therefore $V$, and we
compare the two tuples below:
      %
      \begin{equation*}
      %
	\big(\HE(\epoch),\, \ek',\, \HE(\epoch)^{s},\, \ek'^{s}\big) \quad
\text{versus} \quad \big(\HE(\epoch),\, \ek',\, \HE(\epoch)^{s},\, V\big).
      %
      \end{equation*}
      %
      This is exactly a DDH instance in $\group{1}$, so hardness of DDH in
$\group{1}$ makes the change undetectable, and it does so whether or not the
distinguisher holds $\ek'$. An authorised auditor does hold it, so this matters.
Note that $\Xi$ needs no separate assumption, since it is a function of public
data and of the simulator's own $s$. In particular, no claim about $\group{T}$
is required here.

    \item[Simulating the sigma protocol.] With $(U, V, \Xi, \com_\gamma)$ fixed,
...
\Xi^{\theta_2+\theta_\gamma}\cdot\pairing(V,\ggen_D)^{-c}, \qquad A_\gamma =
...
$(A, B, D, C, F, A_\gamma)$ exactly as above, and then programs $\hash(\inst_7',
A, B, D, C, F, A_\gamma) = c$. Since $\hash$ is a random oracle this is possible
...
\subsection{Proof of~\Cref{thm:inst}}\label{sec:txid-inst-sec}
...
\begin{description}
  ...
  \item[(P1).] $\DS$ is the BBS+ signature scheme of~\Cref{fig:bbs}, which is
existentially unforgeable under the $q$-SDH assumption (\Cref{qsdh-bbs}). The
...
this.
  ...
  \item[(P2).] $\Com$ is the Pedersen commitment scheme of~\Cref{fig:pedersen},
...
any party sample them.
  ...
  \item[(P3).] Condition (F1) is~\Cref{lem:txid-fe-prf}, under DDH in
$\group{1}$ with $\HE$ modelled as a random oracle, with $\ek_\epoch = \ek'$.
Condition (F2) is~\Cref{lem:txid-ft-prf}, under $(\Max,n)$-mDBDHI, with
$\mathsf{aux} = \{W_\gamma\}$. Note that these two lemmas are exactly what Game
4 and Game 5 appeal to, and that no stronger statement about $\FE$ is available
or needed: $\mathsf{aux}$ is identical in every epoch by construction, and the
simulator of Game 4 reproduces that by reusing one $\mathsf{aux}_\user$ per
user.

  \item[(P4).] The proof systems are the sigma protocols
of~\Cref{prot:txid-tag-sigma,prot:txid-tx-nizk}, Fiat-Shamir transformed with
$\hash$ modelled as a random oracle, the Schnorr proof of representation used
for $\rel_4$, the batched proof for the $\{W_\gamma\}$ clause of $\rel_5$, and
the Bulletproofs range argument~\cite{SP:BBBPWM18}, whose soundness rests on
discrete logarithm in $\group{1}$. That the rewritten relations prove what the
