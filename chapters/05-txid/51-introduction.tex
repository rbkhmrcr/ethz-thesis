...
systems in this chapter must satisfy three properties: balance, meaning that no
money can be created by transactions; theft prevention, meaning that no user of
the system can spend money belonging to another; and privacy, which we define
formally in~\Cref{sec:txid-definitions} and which is an intrinsic property of
...
prevention: the confidential payload of a transaction in our scheme is exactly
the transaction of the underlying system, carried through unchanged. What we add
is a way for a designated auditor to recover which of those payloads belong to a
given user, and only during a window that the user has agreed to.
...
counter of prior work forbids. The design gives users fine-grained control
...
permission to be issued for any subsequent one. We formalise tracing as an ideal
...
signature scheme to let the registration authority certify a user's keys without
ever learning their tracing key.
...
\Cref{sec:txid-instantiation} gives a concrete instantiation in pairing-based
groups, proved secure in \Cref{sec:txid-security}.  \Cref{sec:txid-evaluation}
evaluates the concrete instantiation, and \Cref{sec:txid-summary} concludes.
