...
systems in these chapters must satisfy three properties. Balance, which means
that no money can be created by transactions.  Theft prevention, which means
that no user of the system can spend money belonging to another, and finally,
privacy. We will define privacy formally in the definitions sections, but note
here that it is an intrinsic property of the transaction systems we are working
with.
...
We must first distinguish between fiat and non-fiat transaction
systems\footnote{Fiat is Latin for `let it be done'.}. Fiat money is issued by
an entity such as a central bank. The backing of the state gives the currency
its value and so its issuance is centralised by design. Fiat money exists in
different forms. Those we are interested in are cash, the current banking
system, and the banking system of the near future.
...
Fiat systems have a single issuer controlling both monetary policy and
validation.  Cryptocurrencies replace this issuer with a public ledger, whose
validators follow the protocol for economic rather than institutional reasons.
This allows the system to remain permissionless, and this in turn is how
cryptocurrencies function with participants with no registered identities. In
fiat systems, behaviour is enforced not by protocol level incentive
compatibility, but by the ability of regulators and legal authorities to act
against malicious parties. This requires participants to be identifiable, and
so introduces the requirement of registered identities.
...
technical distinction between fiat and non-fiat money is the treatment of
identity.
...
other hidden user identifier to each transaction, so that an auditor holding
...
monotonically increasing counter, and use zero-knowledge proofs to show the
tags are correctly formed and bound to the transaction's
origin~\cite{CCS:KiaKohSar22,CANS:SarKiaKoh24}.  Given the tracing secret of a
target user, an auditor identifies the corresponding transactions by
recomputing the tags and matching them against the ledger, and the counter
binding also stops a user later repudiating their own transactions. These
systems are necessarily tied to users' identities.  All of these share the same
limitation: a user's transactions must be generated sequentially, which
prevents concurrent transaction creation and degrades performance in
high-throughput settings.

Cryptocurrencies generally provide auditability through interactive protocols
or global consistency checks. ZKLedger lets auditors verify transaction
